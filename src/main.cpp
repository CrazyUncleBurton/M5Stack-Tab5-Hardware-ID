...
void setup()
{
  ...
  y += lineHeight;
  display.setCursor(x, y);
  display.printf("LCD Resolution: %dx%d\n", M5.Display.width(), M5.Display.height());
  y += lineHeight;
  display.setCursor(x, y);
  display.printf("LCD Max Touch Points: %d\n", m5::Touch_Class::TOUCH_MAX_POINTS);
  y += lineHeight;
  display.setCursor(x, y);
  display.printf("MCU: %s Rev%d @ %dMHz\n", ESP.getChipModel(), ESP.getChipRevision(), ESP.getCpuFreqMHz());
  y += lineHeight;
  display.setCursor(x, y);
  display.printf("Flash: %u MB\n", ESP.getFlashChipSize()/(1024*1024));
  y += lineHeight;
  display.setCursor(x, y);
  display.printf("PSRAM: %u MB\n", ESP.getPsramSize()/(1024*1024));
  y += lineHeight;
  display.setCursor(x, y);
